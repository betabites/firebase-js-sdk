...
describe('Firebase Storage > Requests', () => {
  ...
  it('getBytes handler', async () => {
    ...
    const bytes = await requestInfo.handler(
      fakeResponse(new Uint8Array([1, 128, 255]), {})
  ...
  it('error handler passes through unknown errors', () => {
    const requestInfo = getMetadata(storageService, locationNormal, mappings);
    const error = unknown();
    const resultError = requestInfo.errorHandler!(fakeResponse("", {}, 509), error);
    assert.equal(resultError, error);
  });
  it('error handler converts 404 to not found', () => {
    const requestInfo = getMetadata(storageService, locationNormal, mappings);
    const error = unknown();
    const resultError = requestInfo.errorHandler!(fakeResponse("", {}, 404), error);
    assert.isTrue(resultError._codeEquals(StorageErrorCode.OBJECT_NOT_FOUND));
  });
  it('error handler converts 402 to quota exceeded', () => {
    const requestInfo = getMetadata(storageService, locationNormal, mappings);
    const error = unknown();
    const resultError = requestInfo.errorHandler!(fakeResponse("", {}, 402), error);
